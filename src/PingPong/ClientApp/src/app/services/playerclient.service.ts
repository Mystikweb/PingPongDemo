...
export class PlayerClientService {
    ...
    protected processGetAll(response: HttpResponseBase): Observable<Player[] | null> {
        ...
        return _observableOf<Player[] | null>(<any>null);
    }

    create(player: Player | null): Observable<void> {
        let url_ = this.baseUrl + "/api/Player";
        url_ = url_.replace(/[?&]$/, "");
    ...
    getById(id: number): Observable<Player | null> {
    ...
    protected processGetById(response: HttpResponseBase): Observable<Player | null> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = resultData200 ? Player.fromJS(resultData200) : <any>null;
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
    ...
    update(id: number, player: Player | null): Observable<void> {
        let url_ = this.baseUrl + "/api/Player/{id}";
        if (id === undefined || id === null)
            throw new Error("The parameter 'id' must be defined.");
        url_ = url_.replace("{id}", encodeURIComponent("" + id));
